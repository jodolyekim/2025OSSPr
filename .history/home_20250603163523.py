...
from country_filtering import (
    ...
    get_language_code,
    translate_text  # 아래에서 새롭게 수정한 deep-translator 기반 함수 사용
)
...
if contents:
    ...
    for tab, media_type in zip(tabs, ["movie", "tv"]):
        with tab:
            ...
            original_overview = content.get("overview", "-")
            st.info(f"🇰🇷 {original_overview}")

            if overview_local and overview_local != original_overview:
                st.info(f"🌐 ({target_lang.upper()}) {overview_local}")
            else:
                # TMDB 번역이 없을 경우 deep-translator 번역 대체
                translated = translate_text(original_overview, target_lang)
                if translated and translated != original_overview:
                    st.info(f"🌐 ({target_lang.upper()}) {translated}")
                            ...
                            with cols[1]:
                                if monetization == "flatrate":
                                    ...
                                    if platform.get("price"):
                                        ...
                                        st.markdown(f"- {platform_name.title()}")
                                # 추천 컨테츠 표시 및 fallback
                                st.markdown("---")
                                st.subheader("🤖 이 컨테츠와 비슷한 추천작")

                                similar = st.session_state["recommender"].get_similar(content["id"], top_k=5, weights=None, min_score=30)  # 보유 된 버전의 평균적 추천 무료
                                ...
                                if similar:
                                    ...
                                    st.warning("⛔️ 이 컨테츠와 유사한 컨테츠를 찾을 수 없습니다.")
                                    st.info("⬇️ 다른 추천 기능을 이용해 보세요:")
                                    col1, col2 = st.columns(2)
                                    with col1:
                                        st.page_link("pages/1_calc_page.py", label="📊 가성비 계산기로 이동", icon="📈")
                                    with col2:
                                        st.page_link("pages/2_mbti_page.py", label="🤠 MBTI 추천기로 이동", icon="🔮")