...
if contents:
    ...
    for tab, media_type in zip(tabs, ["movie", "tv"]):
        with tab:
            ...
            if media_type == "tv":
                st.write("🧪 선택된 TV 콘텐츠:", content)
                st.write("📅 간편 검색의 first_air_date:", content.get("first_air_date", "없음"))

                details_debug = get_details(content["id"], "tv")
                st.write("📅 상세정보의 first_air_date:", details_debug.get("first_air_date", "없음"))
