...
if "movie" in locals() and movie is not None and "title_ko" in movie:
    if len(st.session_state["selected_movies"]) >= 10:
        st.warning("영화는 최대 10개까지 선택할 수 있습니다. 가성비 추천기에서 결과를 확인해주세요.")
        st.markdown("[👉 가성비 추천기 페이지로 이동](http://localhost:8501/cost_effectiveness)")
    else:
        if st.checkbox(f"📌 '{movie['title_ko']}' 이(가) 가성비 계산기에 추가하기", key=movie["title_ko"]):
...
# ✅ 영화가 하나라도 선택된 경우, 명확한 이동 버튼 제공
if st.session_state["selected_movies"]:
    st.markdown("---")
    st.markdown(f"🎯 현재 선택된 영화 수: **{len(st.session_state['selected_movies'])}개**")
    st.markdown("[👉 가성비 추천기 결과 보기](http://localhost:8501/cost_effectiveness)")
if st.session_state["selected_movies"]:
