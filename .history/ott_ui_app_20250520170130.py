...
if st.button("검색"):
    with st.spinner("검색 중..."):
        ...
        st.session_state["movie_result"] = movie # 
...
# ✅ 검색 후 선택 UI가 유지되지 않는 문제 해결용
# (기존 코드에서 검색한 movie를 세션에 저장했다는 가정)
if "movie_result" in st.session_state and st.session_state["movie_result"]:
    ...
    # 추가 기능 안내
    st.markdown("### 📌 가성비 계산기 추가 기능")

    # 조건에 따라 버튼 출력 또는 상태 안내
    if len(st.session_state["selected_movies"]) >= 10:
        st.warning("⚠️ 영화는 최대 10개까지 선택할 수 있습니다.")
    elif movie["title_ko"] in st.session_state["selected_movies"]:
        st.info(f"✅ 이미 '{movie['title_ko']}' 은(는) 추가된 상태입니다.")
    else:
        if st.button(f"📌 '{movie['title_ko']}' 가성비 계산기에 추가하기"):
            st.session_state["selected_movies"].append(movie["title_ko"])
            st.success(f"✅ '{movie['title_ko']}' 이(가) 가성비 계산기에 추가되었습니다.")

    # ✅ 선택된 영화가 하나라도 있으면 버튼 + 목록 출력
    if st.session_state["selected_movies"]:
        st.markdown("---")
        st.markdown(f"🎯 현재 선택된 영화 수: **{len(st.session_state['selected_movies'])}개**")

        # Streamlit 공식 구조: 버튼 대신 페이지 링크 (st.page_link는 1.32 이상부터 가능)
        st.markdown(
            """
            <a href="/1_cost-effectiveness" target="_self">
                <button style='font-size:16px;padding:8px 16px;background-color:#4CAF50;color:white;border:none;border-radius:5px;cursor:pointer;'>
                    📊 가성비 계산기 결과 보기
                </button>
            </a>
            """,
        ...
        with st.expander("🧾 선택된 영화 목록 펼쳐보기"):
            for i, title in enumerate(st.session_state["selected_movies"], 1):
