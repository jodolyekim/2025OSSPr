...
def extract_year(c, media_type):
    ...
    return date[:4] if date and len(date) >= 4 else "N/A"
...
if 'selected_movie_id' in st.session_state:
    ...
    # 1. get_details로 기본 정보 가져오기
    detail = get_details(movie_id, media_type="movie")

    # 2. 다시 title 기준으로 TMDB에서 한글 제목 포함된 결과 요청
    search_result = search_movie_tmdb(detail.get("title", ""))
    matched = next((c for c in search_result if str(c["id"]) == str(movie_id)), None)

    # 3. 최종 콘텐츠 구성 (한글 정보 + 세부정보 병합)
    contents = [{
        "id": movie_id,
        "title_ko": matched.get("title_ko", detail.get("title", "")) if matched else detail.get("title", ""),
        "title_en": detail.get("title", ""),
        "release_date": matched.get("release_date", detail.get("release_date", "")) if matched else detail.get("release_date", ""),
        "overview": matched.get("overview", detail.get("overview", "")) if matched else detail.get("overview", ""),
        "poster_path": matched.get("poster_path", detail.get("poster_path", "")) if matched else detail.get("poster_path", ""),
        "vote_average": matched.get("vote_average", detail.get("vote_average", 0)) if matched else detail.get("vote_average", 0),
        "media_type": "movie"
    }] if detail else []
    title = contents[0]["title_ko"] if contents else ""
    ...
    for tab, media_type in zip(tabs, ["movie", "tv"]):
        with tab:
            ...
            for c in filtered:
                ...
                # ✅ media_type에 따라 날짜 필드 다르게 가져오기
                raw_date = c.get('release_date') if media_type == 'movie' else c.get('first_air_date')
        ...
        with st.container():
            ...
            with cols[0]:
                if len(st.session_state['selected_contents']) >= 10:
                    ...
                    title_to_add = content.get('title_ko', '제목없음')
